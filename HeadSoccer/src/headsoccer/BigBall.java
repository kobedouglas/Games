 ...
 * @author Kobe Douglas
 */
public class BigBall extends PowerUps {
     ...
     */
    public BigBall(double canW, double canH, double goalW, double goalH, double w, double h){
        super(Math.random() * (canW - 2 * goalW - w) + goalW, Math.random() * (canH - goalH - (canH / 2 - (w/2))) + (canH / 2 - (w/2)) - w, w, h, Color.GOLDENROD);
        setXvel(0);
        setYvel(0);
    }
     ...
     * @param h is the height of the power up
     */
    public BigBall(double canW, double canH, double goalW, double goalH, Color c, Image p, double w, double h){
