 ...
 * @author Kobe Douglas
 */
public class SmallBall extends PowerUps {
     ...
     */
    public SmallBall(double canW, double canH, double goalW, double goalH, double w, double h){
        super(Math.random() * (canW - 2 * goalW - w) + goalW, Math.random() * (canH - goalH - (canH / 2 - (w/2))) + (canH / 2 - (w/2)) - w, w, h, Color.HOTPINK);
        setXvel(0);
        setYvel(0);
    }
     ...
     * @param h is the height of the power up
     */
    public SmallBall(double canW, double canH, double goalW, double goalH, Color c, Image p, double w, double h){
